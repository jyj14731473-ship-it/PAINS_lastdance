...
CONFIG = {
    "features": ["acute_workload_7d", "chronic_workload_28d"],
    "lam": 0.6,
    "n_splines": 12,
}
...
def _fit_gam(x_train, y_train, sample_weight, cfg):
    try:
        ...
        terms = sum([s(i, n_splines=int(cfg["n_splines"])) for i in range(x_train.shape[1])])
        model = LinearGAM(terms, lam=float(cfg["lam"]))
...
def run(config: dict, train_df, test_df) -> dict:
    ...
    if not features:
        raise ValueError("model_gam_raw has no available workload features.")
